...
public class Pojo {
	...
	@Pattern(regexp = "^\\d{4}-(0[1-9]|1[0-2])-(0[1-9]|[12]\\d|3[01])\\s([01]\\d|2[0-3]):[0-5]\\d:[0-5]\\d$", message = "Date of birth must be in format yyyy-MM-dd HH:mm:ss")
	@JsonProperty(value = "dateOfBirth", access = Access.READ_WRITE)
	@JsonView({ Post.class, Get.class, Put.class })
	private String dateOfBirth;

	@Size(min = 8, max = 12, message = "Password must be between 8 and 12 characters")
	@Pattern(regexp = "^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d)(?=.*[@$!%*?&])[A-Za-z\\d@$!%*?&]{8,12}$", message = "Password must contain at least one uppercase, one lowercase, one number, and one special character")
	@JsonProperty(value = "password", access = Access.READ_WRITE)
	@JsonView({ Get.class })
	private String password;

	@Digits(groups = { Post.class,
			Put.class }, integer = 7, fraction = 2, message = "Only up to max 7 digits with 2 decimal places")
	@DecimalMin(groups = { Post.class,
			Put.class }, value = "0.00", inclusive = true, message = "Account balance must be larger or equal to 0.00")
