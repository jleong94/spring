...
/**
 * A configuration component that holds rate limit settings (bandwidth rules)
 * for different API paths.
 * 
 * Each path is mapped to a {@link Bandwidth} configuration, which defines the
 * number of allowed requests
 * and how tokens are refilled over time.
 * 
 * Uses Bucket4j's Bandwidth to implement token bucket rate limiting per
 * endpoint.
 */
@Configuration
...
public class RateLimitProperties {
    ...
    public void init() {
        ...
        // Endpoints
        endpoints.forEach((path, rate) -> limits.put(path, Bandwidth.builder()
